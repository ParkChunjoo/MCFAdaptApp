...
namespace MCFAdaptApp.Avalonia.Views
{
    public partial class SelectPatientView : UserControl
    {
        ...
        private void DataGrid_SelectionChanged(object? sender, SelectionChangedEventArgs e)
        {
            if (DataContext is SelectPatientViewModel viewModel)
            {
                if (viewModel.SelectedPatient != null)
                {
                    ...
                    // If this patient has structures, fetch them
                    if (viewModel.SelectedPatient.Structures == null || !viewModel.SelectedPatient.Structures.Any())
                    {
                        Task.Run(async () =>
                        {
                            try
        ...
        private async void SelectReferencePlanButton_Click(object? sender, RoutedEventArgs e)
        {
            ...
            LogHelper.Log("SelectReferencePlan_Click: Phase 1 - Processing.");
            // No delay - immediate processing
            LogHelper.Log("SelectReferencePlan_Click: Phase 1 - Processing finished.");
            ...
            LogHelper.Log("SelectReferencePlan_Click: Phase 2 - Processing.");
            // No delay - immediate processing
            LogHelper.Log("SelectReferencePlan_Click: Phase 2 - Processing finished.");
                ...
                if (visualRoot is MainWindow mainWindow)
                {
                    ...
                    if (registerView != null && registerView.DataContext is RegisterViewModel registerViewModel && DataContext is SelectPatientViewModel viewModel)
                    {
                        ...
                        if (patient != null)
                        {
                            ...
                            RTStructure? structure = null;
                            RTPlan? plan = null;

                            // Find which structure contains this plan
                            if (patient.Structures != null)
                            {
                                // Get the button that was clicked
                                if (sender is Button button && button.DataContext is RTPlan clickedPlan)
                                {
                                    plan = clickedPlan;
                                    LogHelper.Log($"SelectReferencePlan_Click: Got RTPlan");
                                    if (plan != null)
                                    {
                                        LogHelper.Log($"SELECTED TREATMENT PLAN: {plan.Name}");
                                    }
                                    else
                                    {
                                        LogHelper.Log("SELECTED TREATMENT PLAN: None");
                                    }

                                    // Find which structure contains this plan
                                    foreach (var rtStructure in patient.Structures)
                                    {
                                        if (rtStructure.Plans != null && rtStructure.Plans.Contains(clickedPlan))
                                        {
                                            structure = rtStructure;
                                            LogHelper.Log($"SelectReferencePlan_Click: Got RTStructure: {structure.Name}");
                                            break;
                                        }
                                    }

                                    if (structure == null && patient.Structures.Count > 0)
                                    {
                                        LogHelper.LogWarning("SelectReferencePlan_Click: No Plans found in the RTStructure.");
                                        LogHelper.Log("SELECTED TREATMENT PLAN: None");
                                    }
                                }
                                ...
                                mainWindow.UpdatePatientInfo(patient, structure, plan);

                                // Initialize the RegisterViewModel with the patient, structure, and plan
                                LogHelper.Log("SelectReferencePlan_Click: Initializing RegisterViewModel with full objects.");
                                await registerViewModel.InitializeAsync(patient, structure, plan);
                            }
                            else
                            {
                                LogHelper.LogWarning("SelectReferencePlan_Click: No Structures found for the Patient.");
                                LogHelper.Log("SELECTED TREATMENT PLAN: None");
